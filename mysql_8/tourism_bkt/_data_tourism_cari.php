...
if ($tipe == 1) {
	...
	left JOIN tourism ON ST_Distance_Sphere(ST_Centroid(hotel.geom), ST_Centroid(tourism.geom))
...
if(isset($qswp)){
	...
	while($baris = mysqli_fetch_assoc($hasil2)){
		if($baris['id'] != NULL){
			...
			$lng=$baris['lon'];
			$lat=$baris['lat'];
			$dataarray[]=array('idm'=>$id,'namem'=>$name,'lngm'=>$lng,'latm'=>$lat);
		}
	}
}
